...
const state = await p.evaluate(() => {
  ...
  return {
    ...
    okRoot: vis(".ok-root"),
    phone: {
      matchesPhone: matchMedia("(max-width:640px)").matches,
      okVh: getComputedStyle(document.documentElement).getPropertyValue("--ok-vh").trim(),
      okKb: getComputedStyle(document.documentElement).getPropertyValue("--ok-kb").trim(),
      ...
      rootHeightPx: (() => { const e = document.querySelector(".ok-root"); return e ? Math.round(e.getBoundingClientRect().height) : null; })(),
      topBarHeight: (() => { const e = document.querySelector(".ok-top"); return e ? Math.round(e.getBoundingClientRect().height) : null; })(),
      firstTile: (() => { const e = document.querySelector(".ok-tile"); if (!e) return null; const r = e.getBoundingClientRect(); return { w: Math.round(r.width), h: Math.round(r.height), top: Math.round(r.top) }; })(),
    },
